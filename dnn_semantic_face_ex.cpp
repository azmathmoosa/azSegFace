...
#include "dnn_semantic_segmentation_net.h"
...
dlib::rgb_pixel operator && (const dlib::rgb_pixel& a, const dlib::rgb_pixel& b)
{
    return rgb_pixel(a.red * b.red, a.green * b.green, a.blue * b.blue);
}

matrix<rgb_pixel> mask_image(const matrix<rgb_pixel>& image, const matrix<rgb_pixel>& mask)
{
    matrix<rgb_pixel> masked_image(image.nr(), image.nc());
    masked_image = rgb_pixel(255,255,255);
    for (long i=0; i < mask.nr(); i++)
    {
        for (long j=0; j< mask.nc(); j++)
        {
            masked_image(i,j) = image(i,j) && mask(i,j);
            if (masked_image(i,j) == rgb_pixel(0,0,0))
                masked_image(i,j) = rgb_pixel(255,255,255);
        }
    }
    return masked_image;
}
...
int main(int argc, char** argv) try
{
    if (argc != 2)
    {
        ...
        cout << "./dnn_semantic_face_ex /path/to/images" << endl;
        cout << endl;
        cout << "You will also need a trained 'semantic_face_net.dnn' file." << endl;
    ...
    // Read the file containing the trained network from the nets directory.
    load_net("..\\..\\nets\\semantic_face_net.dnn");
    // anet_type net;
    // deserialize("semantic_face_net.dnn") >> net;
    ...
    for (const file& file : files)
    {
        ...
        const matrix<uint16_t> temp = infer_net(input_image);
        ...
        dlib::matrix<rgb_pixel> masked_image = mask_image(input_image,rgb_label_image);

        // Show the input image on the left, and the predicted RGB labels on the right.
        win.set_image(join_rows(input_image, masked_image));

        
        //rgb_label_image = ceil()
        //auto mask = pointwise_multiply(rgb_label_image, input_image)
